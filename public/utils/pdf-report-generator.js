...
function buildLLMChanges(srxTranslatedPolicies, originalPolicies, isLocalLLM, llmModel = '') {
  ...
  const llmLabel = (isLocalLLM ? 'Local LLM' : 'Cloud LLM') + (llmModel ? ` (${esc(llmModel)})` : '');
...
export function generateFullPdfHtml(data) {
  ...
  const sections = [
    ...
    section(4, 'LLM Changes', buildLLMChanges(srxTranslatedPolicies, originalPolicies, isLocalLLM, llmModel), { accent: isLocalLLM ? C.llmLocal : C.llmCloud }),
...
<div class="report-header">
  ...
  <div class="subtitle">${esc(vendorLabel)}${sourceModel ? ' (' + esc(sourceModel) + ')' : ''} &rarr; <span class="target-platform">Juniper SRX${targetModel ? ' (' + esc(targetModel) + ')' : ''}</span>${siteName ? ' | Site: ' + esc(siteName) : ''}${siteGroup ? ' (' + esc(siteGroup) + ')' : ''} | Generated: ${esc(now)} | Tool v${esc(APP_VERSION)}${llmModel ? ' | LLM: ' + esc(llmModel) : ''}</div>
</div>
${sections.join('\n')}
<div class="report-footer">Generated by ${BRAND.product} v${esc(APP_VERSION)}${llmModel ? ' &middot; LLM: ' + esc(llmModel) : ''} &middot; ${BRAND.endorsement} &mdash; ${esc(now)}</div>
